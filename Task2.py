...
def create_init_pos(N,R, min_distance = 1):
    r_arr = np.zeros((N, 2))
    v_arr = np.zeros((N, 2))
    v_arr[:,1] = 5
...
def test_code_const_energy():
    ...
    #plt.plot(t_arr1, E1, label=f'N={N1}, R={R1}, dt={dt1}', color = 'cornflowerblue')
    #plt.plot(t_arr2, E2, label=f'N={N2}, R={R2}, dt={dt2}', color= 'darksalmon')
    plt.plot(t_arr3, E3, label=f'N={N3}, R={R3}, dt={dt3}', color = 'mediumseagreen')
